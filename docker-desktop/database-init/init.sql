...
INSERT IGNORE INTO `system_config` (`name`, `title`, `value`) VALUES ('mobileShowSize', '移动端显示文件大小', 'true');

-- ----------------------------
-- Records of user
-- ----------------------------
INSERT IGNORE INTO `user` VALUES (0, 'template', '虚拟新用户', NULL, b'1', now(), now(), NULL, NULL);
INSERT IGNORE INTO `user` VALUES (1, (SELECT `value` FROM `system_config` WHERE `name` = 'username'), '管理员', (SELECT `value` FROM `system_config` WHERE `name` = 'password'), b'1', current_timestamp, NULL, NULL, NULL);
INSERT IGNORE INTO `user` VALUES (2, 'guest', '匿名用户', NULL, b'1', current_timestamp, NULL, NULL, NULL);
